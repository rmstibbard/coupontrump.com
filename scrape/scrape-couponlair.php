...
$site = "www.couponlair.com";

$endings = array ("/Discount/", "/Bargains/", "/Free/");
...
foreach ($endings as $ending ) {
    ...
    for ($i=$from; $i<=$to; $i++) {
        ...
        $results_page = scrape_between($results_page, '<html lang="en">', '</html>');
        
        $separate_results = explode('<div class="col-xs-6 col-sm-3">', $results_page);
        
        foreach ($separate_results as $separate_result) {
            
            if ($separate_result != "") {
            
                $base_url = trim(scrape_between($separate_result, '<a href="/s/', '" rel="nofollow"'));
                
                if ($base_url!="") {
                    $link_url = "http://www.couponlair.com/s/" . $base_url;
